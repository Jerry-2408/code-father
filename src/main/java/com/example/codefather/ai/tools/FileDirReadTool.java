...
public class FileDirReadTool extends BaseTool {
    ...
    public String readDir(
            @P("目录的相对路径，为空则读取整个项目结构") String relativeDirName,
            @ToolMemoryId Long appId
    ) {
        try {
            Path DirPath = Paths.get(relativeDirName == null ? "" : relativeDirName);
            // 构建绝对路径
            if (!DirPath.isAbsolute()) {
                String projectDirName = "vue_project_" + appId;
                Path projectRoot = Paths.get(AppConstant.CODE_OUTPUT_ROOT_DIR, projectDirName);
                DirPath = projectRoot.resolve(relativeDirName == null ? "" : relativeDirName);
            }
            File targetDir = DirPath.toFile();
            if (!targetDir.exists() || !targetDir.isDirectory()) {
                return "错误：目录不存在或不是目录 - " + relativeDirName;
            }
            ...
            String errorMessage = "读取目录结构失败: " + relativeDirName + ", 错误: " + e.getMessage();
    ...
    public String generateToolExecuteResult(JSONObject arguments) {
        String relativeDirName = arguments.getStr("relativeDirName");
